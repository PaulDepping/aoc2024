...
fn add_positions_for_pairs<'a>(
    ...
    for other_position in iter {
        ...
        {
            let row = other_position.row as isize + row_dif;
            let column = other_position.column as isize + column_dif;
        ...
        {
            let row = position.row as isize - row_dif;
            let column = position.column as isize - column_dif;
            if (0..row_max).contains(&row) && (0..column_max).contains(&column) {
                valid_positions.insert(Position {
                    row: row.try_into()?,
                    column: column.try_into()?,
