...
fn check_sum(
    ...
    if let Some(next_val) = elements_iter.next() {
        ...
        let sum_concat = format!("{}{}", current_sum, next_val).parse().unwrap();

        let current_state = elements_iter.clone();
        let current_state2 = elements_iter.clone();
        check_sum(total_sum, sum_add, elements_iter)
            || check_sum(total_sum, sum_mul, current_state)
            || check_sum(total_sum, sum_concat, current_state2)
    } else {
